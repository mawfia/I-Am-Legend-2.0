...
<%-- 			<p>${keyList[0]}</p> --%>
	...
	<c:choose>
		<c:when test="${ empty account }">
			...
			<tr>
				<td><a href="admin.do?acct=${users.get(keyList[0]).accountNumber}&operation=1">Update</a> <a href="admin.do?acct=${users.get(keyList[0]).accountNumber}&operation=3">Remove</a></td>
       			<td>${users.get(keyList[0]).accountNumber}</td>
       			<td>${users.get(keyList[0]).firstName}</td>
       			<td>${users.get(keyList[0]).lastName}</td>
       			<td>${users.get(keyList[0]).zipcode}</td>
       			<td>${users.get(keyList[0]).age}</td>
       			<td>${users.get(keyList[0]).height}</td>
       			<td>${users.get(keyList[0]).weight}</td>
    		</tr>
    		</table>
    	</c:when>
    	<c:otherwise>
	...
	<form action="admin.do" method="POST">
		...
		<datalist id="json-datalist"> 
			<c:forEach var="account" items="${users.values()}">
...
<!-- 		<button name="operation" value="3">Remove</button> -->
	...
	<fieldset>
		...
		<table>
			...
			<tbody>
			<c:forEach var="accounts" items="${users.values()}">
				<tr>
					...
					<td><a href="profile.jsp?acct=${accounts.accountNumber}&operation=3">Remove</a></td>
				</tr>
			</c:forEach>
			</tbody>
			<tfoot></tfoot>
		...
		<form action="profile.jsp" method="POST"><input type="submit" value="Profile"></form>
